 ...
 */
import { api, state, AuthRequiredError } from './api.js';
...
function buildFooterActions() {
  const ownerBtn = state.ownerGateEnabled
    ? el('button', {
        class: 'nav-item',
        onclick: async () => {
          if (state.isOwner) {
            try {
              ...
              toast('Signed out.');
              paintNav();
              if (routeOf() === '/dashboard') renderRoute();
            } catch (e) { toast(e.message, 'error'); }
          } else {
            promptOwnerLogin(() => { paintNav(); renderRoute(); });
          }
        },
      }, el('span', { class: 'ico' }, state.isOwner ? '🔓' : '🔒'),
        el('span', {}, state.isOwner ? 'Sign out (owner)' : 'Sign in as owner'))
    : null;
  ...
  return el('div', { class: 'stack-sm' },
    ...
    }, el('span', { class: 'ico' }, '♻️'), el('span', {}, 'Reset demo data')) : null,
  );
}
...
function paintPersona() {
  clear(personaHost);
  // The persona switcher lets you view as any customer. That is a demo feature
  // and a privacy hole in production, so it disappears when demo_mode is off.
 ...
 * Owner login prompt.
 *
 * The dashboard is password-protected on a public domain, so a 401 surfaces
 * this rather than an error toast — the user is not doing anything wrong, they
 * just have not signed in yet.
 */
function promptOwnerLogin(onSuccess) {
  const input = el('input', { class: 'input', type: 'password', placeholder: 'Owner password' });
  const submit = async () => {
    if (!input.value) { toast('Enter the password.', 'error'); return false; }
    try {
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { const r = submit(); if (r !== false) host.close(); }
  });
  const host = modal({
    title: '🔒 Owner access',
    body: el('div', { class: 'stack' },
      el('p', { class: 'small muted' },
        'The owner console shows your takings, customer names and phone numbers, so it sits '
        + 'behind a password. This is the password set during installation.'),
      el('div', { class: 'field' }, el('label', {}, 'Password'), input)),
    actions: [
      { label: 'Cancel', class: 'btn-ghost' },
      { label: 'Sign in', class: 'btn-primary', onClick: submit },
    ],
  });
}
...
async function renderRoute() {
  ...
  try {
    ...
    if (e instanceof AuthRequiredError || e?.authRequired) {
      clear(contentHost);
      contentHost.append(empty('🔒', 'Owner password needed',
        'This screen is protected. Sign in to continue.'));
      contentHost.append(el('div', { class: 'center', style: { marginTop: '14px' } },
        el('button', {
          class: 'btn btn-primary',
          onclick: () => promptOwnerLogin(() => renderRoute()),
        }, 'Sign in as owner')));
      promptOwnerLogin(() => renderRoute());
      return;
    }
    clear(contentHost);
