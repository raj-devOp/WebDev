...
export async function render(host) {
  ...
  const sel = {
    ...
    date: todayISO(),
    time: null,
    autoAdvanced: false,   // only ever roll forward once, so we cannot loop
    addonIds: new Set(),
    payWithWallet: true,
  };
  ...
  async function loadSlots() {
    ...
    // If today has nothing left (common in the evening, once closing time has
    // passed) roll forward to the next day rather than presenting a grid where
    // every slot is greyed out. Only once, so a genuinely full week still shows
    // the message below instead of skipping through dates.
    if (!anyFree && !sel.autoAdvanced && sel.date === todayISO()) {
      sel.autoAdvanced = true;
      sel.date = new Date(Date.now() + 86400000).toISOString().slice(0, 10);
      sel.time = null;
      paintDates();
      paintBasket();
      return loadSlots();
